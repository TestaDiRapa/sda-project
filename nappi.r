...
data.plot$ridge.pred <- ridge.pred
mean((ridge.pred-y_test)^2)

lasso.pred=predict(lasso.mod,s=bestlaml,x_test)
data.plot$lasso.pred <- lasso.pred
...
data.plot$ridge7.pred <- ridge7.pred
mean((ridge7.pred-y_test)^2)

lasso7.pred=predict(lasso7.mod,s=bestlaml,x_test)
data.plot$lasso7.pred <- lasso7.pred
mean((lasso7.pred-y_test)^2)

data.plot$best.fit = predict(best.fit, data_test)
data.plot$adj.fit <- predict(adj.fit, data_test)
...
test_multiple_country <- function(df.test, iso) {
  ...
  plot <- ggplot(to.plot, aes(x=date)) +
    ...
    geom_line(aes(y=ridge.pred, colour='red', group = 2), size=line.width) +
    geom_line(aes(y=lasso.pred, colour='cyan', group = 3), size=line.width) +
    geom_line(aes(y=ridge7.pred, colour='blue', group = 4), size=line.width) +
    geom_line(aes(y=lasso7.pred, colour='green', group = 5), size=line.width) +
    geom_line(aes(y=best.fit, colour='magenta', group = 6), size=line.width) +
    geom_line(aes(y=adj.fit, colour='yellow', group = 7), size=line.width) + 
    scale_color_discrete(name = "Legend", labels = c("real_cases", "ridge.pred", "lasso.pred", "ridge7.pred", "lasso7.pred","best.fit","adj.fit")) +
...
test_multiple_country(data.plot, 'KOR')
test_multiple_country(data.plot, 'USA')
test_multiple_country(data.plot, 'URY')
test_multiple_country(data.plot, 'IRN')
...
single_state_prediction <- function(country, split) {
  ...
  adj.fit.s <- lm(new_cases~.-aged_70_older-median_age-cvd_death_rate-population_density-gdp_per_capita-female_smokers-population-aged_65_older-diabetes_prevalence-male_smokers, data=s.train)
  ...
  for(i in (split+1):length(pred)) {
    s.test[i,]$actual_cases <- pred[i-1] + s.test[i-1,]$actual_cases
...
single_state_prediction('KOR', 50)
single_state_prediction('USA', 50)
single_state_prediction('URY', 50)
single_state_prediction('ITA', 50)
single_state_prediction('IRN', 25)
